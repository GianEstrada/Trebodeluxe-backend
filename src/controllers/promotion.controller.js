    ...
    try {
      ...
      if (checkResult.rows.length > 0) {
        ...
        if (!promo.porcentaje_descuento) {
          console.log('❌ [REPAIR] Falta registro en promo_porcentaje. Esta promoción necesita ser configurada manualmente por el administrador.');
          
          res.json({
            success: false,
            message: 'Promoción "ea" requiere configuración manual - sin porcentaje definido',
            data: {
              promocion: promo,
              accion_requerida: 'Configurar porcentaje_descuento manualmente'
            }
          });
        } else {
